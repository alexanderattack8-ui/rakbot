...
local cfg = ini.load({
    settings = { 
        bot_name = "", -- Agar config.txt da bo'lmasa, shu nomni oladi
        token = "", 
        chatid = "", 
        password = "", 
        openai_key = "" 
...
-- GITHUB ORQALI AVTO-YANGILANISH FUNKSIYASI
    ...
    newTask(function()
        ...
        if success and response and response.status_code == 200 then
            ...
            if data and data.version and tonumber(data.version) > script_version then
                ...
                print("[UPDATE] Skript yangilanmoqda, kutib turing...")
                ...
                if dl_success and dl_response and dl_response.status_code == 200 then
                    local f = io.open(getWorkingDirectory() .. "\\scripts\\" .. script_name_file, "w")
                    if f then
...
function askChatGPT(system_prompt, user_text)
    ...
    local success, response = pcall(function() return requests.post(url, {headers = headers, data = json.encode(payload), timeout = 5.0}) end)
    ai_busy = false
            ...
            if is_fast_plus then
                ...
                newTask(function()
                    ...
                    if not final_reply then
                        final_reply = askChatGPT("Siz SA-MP server administratori sifatida o'yinchining savoliga qisqa va aniq javob bering.", rep_text)
                        if not final_reply or final_reply == "" then final_reply = "Assalomu Alaykum, sizni kuzatmoqdaman" end
                    end
                    table.insert(report_queue, {id = rep_id, reply = final_reply, name = rep_name, text = rep_text})
                end)
            end
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    print("\n--- DIALOG KELDI ---")
    print("ID: " .. id .. " | Sarlavha: " .. clean_title)
    ...
    if lower_title:find("xush") or lower_title:find("добро") or lower_title:find("yangilik") or lower_title:find("новости") or lower_title:find("grand mobile") then
        print("[BOT] Info oyna yopildi.")
        sendDialogResponse(id, 1, 0, "")
        return false
    end

    if lower_title:find("avtorizatsiya") or lower_title:find("пароль") then 
        print("[BOT] Parol yuborilmoqda...")
        
        if cfg.settings.password == "" then
            print("[XATO] config.txt da parol yo'q! Bot o'yinga kirmasligi mumkin.")
        end
        ...
        if not is_logged_in then
            ...
            newTask(function()
                wait(4000) 
                print("[BOT] 1-Spavn so'rovi...")
                spawn()
                wait(2000)
                
                print("[BOT] 2-Spavn so'rovi...")
                spawn() 
                wait(3000)
                ...
                sendInput("/sp")
                print("[BOT] Avtomatik kuzatuv (SP) rejimiga o'tildi.")
